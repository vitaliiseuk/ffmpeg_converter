...
#include "../corerender/ffmpeg_dxva2.h"
...
typedef struct
{
	int		       nPacketType;			// ????,??EDIA_PACK_TYPE
	char*	       pPacketBuffer;		// ????????
...
void __stdcall FFM_API_XVRRecorderPutPacket(HANDLE hRecorder, void* pPkt)
{
	...
	//	AVStream *out_aud_strm = recorder->pOutAudStream;
	...
	{
	if (!recorder->isVRecordingFirst && recorder->isARecordingFirst)
	{
	recordingStartAudPTS = packet.pts;
	recordingStartAudDTS = packet.pts;
	recorder->isARecordingFirst = false;
	}
